...
- `04_quantitative_results.tex` — Main results table: rows = methods (baselines + proposed), columns = datasets and metrics (AMI/ARI/ACC for emitter clustering and for mode clustering). Bold the best per column. Report mean $\pm$ std over at least 3 seeds.
