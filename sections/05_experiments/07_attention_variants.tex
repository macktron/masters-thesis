...
RoPE-TOA and Bias remove the emitter tail that joint Vanilla leaves in \cref{tab:exp:vanilla,tab:exp:ablation_em}.
...
Emitter-only Vanilla already sits near that ceiling in \cref{tab:exp:ablation_em} ($\ARI$ $0.999$).
...
Vanilla is the ablation of \cref{tab:exp:ablation_em}.
RoPE-az and RoPE-TOA+Bias have no emitter-only run.

\begin{table}[H]
  \centering
  \small
  \caption{Emitter $\ARI$ for joint and single-task (emitter-only) Vanilla, RoPE-TOA, and Bias. Joint rows are copied from \cref{tab:exp:attn_em}. Vanilla emitter-only is copied from \cref{tab:exp:ablation_em}.}
...
\Cref{tab:exp:bias_coef} reports the median over the $8$ heads and $4$ layers of each stack ($32$ scalars per coordinate), with the first and third quartiles.
The mean is a poor summary here: a few large heads pull it away from the value that most heads actually take.
All branch medians are positive except Bias mode time ($-0.115$).
The typical head therefore penalizes a large \gls{toa} or incidence gap.
Every mode incidence scalar is positive in both models, and the two medians agree ($0.622$ and $0.617$); the Bias mean of $0.877$ was the outlier tail, not the typical head.
...
The trunk median stays near zero, and its quartile interval crosses zero.

RoPE-TOA+Bias keeps the same incidence pattern.
Mode time coefficients that were negative under Bias become positive once rotary encoding already carries $\Delta t$.

\begin{table}[H]
  \centering
  \small
  \caption{Learned bias coefficients after training. Each cell is the median over the $8$ heads and $4$ layers of that stack ($32$ scalars); the brackets are the first and third quartiles. A positive coefficient decreases attention as $\lvert p_i-p_j\rvert$ grows (\cref{eq:method:attn_bias}).}
  \label{tab:exp:bias_coef}
  \providecommand{\medq}[3]{%
    \begin{tabular}[t]{@{}c@{}}
      $#1$\\[-0.12em]
      {\scriptsize $[#2,\,#3]$}
    \end{tabular}%
  ...
  \begin{subtable}[t]{0.48\linewidth}
    ...
    \begin{tabular}{@{}lcc@{}}
      \toprule
      Branch & $\lambda_t$ & $\lambda_{\mathrm{inc}}$ \\
      \midrule
      Trunk   & \medq{-0.018}{-0.077}{0.031} & {---} \\
      Mode    & \medq{-0.115}{-0.295}{-0.005} & \medq{0.622}{0.370}{1.159} \\
      Emitter & \medq{0.197}{0.105}{0.305} & \medq{0.260}{0.169}{0.351} \\
      \bottomrule
    \end{tabular}
  \end{subtable}
  \hfill
  \begin{subtable}[t]{0.48\linewidth}
    ...
    \begin{tabular}{@{}lcc@{}}
      ...
      Branch & $\lambda_t$ & $\lambda_{\mathrm{inc}}$ \\
      \midrule
      Trunk   & \medq{0.034}{-0.018}{0.092} & {---} \\
      Mode    & \medq{0.083}{0.037}{0.148} & \medq{0.617}{0.349}{0.792} \\
      Emitter & \medq{0.155}{0.070}{0.238} & \medq{0.143}{0.083}{0.186} \\
      \bottomrule
    \end{tabular}
  \end{subtable}
\end{table}
