...
An emitter-only RoPE-TOA run matches that joint emitter $\ARI$ (\cref{tab:app:ari_em}); the Vanilla joint tax is therefore specific to standard attention.
Emitter-only Bias does not reach the joint Bias ceiling.
...
\Cref{fig:app:hist_em_all,fig:app:hist_md_all} add the single-task runs of Vanilla, RoPE-TOA, and Bias.
