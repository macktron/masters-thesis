...
\subsection{Joint Versus Emitter-Only}\label{sec:exp:attn_task}
...
\begin{table}[H]
  \centering
  \small
  \caption{Emitter $\ARI$ under the joint objective and under $\Loss_{\mathrm{em}}$ only. Joint rows are copied from \cref{tab:exp:attn_em}. Vanilla emitter-only is copied from \cref{tab:exp:ablation_loss}.}
...
Emitter-only RoPE-TOA matches the joint row.
