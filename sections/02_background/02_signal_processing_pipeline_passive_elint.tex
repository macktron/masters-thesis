...
Implementations differ by band and platform, but the order of work is the same: detect and measure, deinterleave into pulse trains, then analyze mode on each train (\cref{fig:bg:classical_pipeline}).

\begin{figure}[H]
\centering
\input{figures/background/classical_pipeline.tex}
\caption{Classical \gls{elint} chain. Detection and measurement produce a mixed \gls{pdw} recording. Deinterleaving forms one hypothesized pulse train per emitter. Mode analysis then runs on each train rather than on the interleaved list.}
\label{fig:bg:classical_pipeline}
\end{figure}
...
Deployed systems still rely predominantly on non-learning methods.
When trains are sparse and nearly periodic, the \gls{pri} inferred from \gls{toa} differences is a usable timing signature.
Histogram methods such as the \gls{cdif} and the \gls{sdif} accumulate inter-pulse intervals and treat peaks as candidate \glspl{pri} \parencite{wiley2006elint,qu2026radardeinterleavingreview}.
...
When timing is not enough, pulses may be clustered in a joint space of \gls{rf}, pulse width, \gls{aoa}, and related measurements, for example with $k$-means or density-based methods \parencite{ester1996density,qu2026radardeinterleavingreview}.
Those assignments are one later option in that feature space, not the operational baseline.
...
Mode analysis is applied only to trains that deinterleaving has already formed.
An error in sorting is therefore frozen before mode analysis begins.
The classical chain never estimates the emitter partition and the mode partition from the interleaved \gls{pdw} list in a single step.
