...
\begin{figure}[tbp]
  \centering
  \input{figures/background/rope_rotation.tex}
...
Stacking $N$ such layers yields a deep encoder that maps an input sequence $(\vect{x}_1,\ldots,\vect{x}_L)$ to contextualised representations $(\vect{h}^{(N)}_1,\ldots,\vect{h}^{(N)}_L)$.
Dense self-attention scales as $L^{2}$, so the method chapter fixes a window length.
