...
- `01_summary.tex` — One or two paragraphs summarizing the thesis: problem, approach, headline result. No new claims. The joint emitter cost is Vanilla-specific; a joint RoPE-TOA encoder matches emitter-only deinterleaving. Emitter-only Bias does not match its joint scores. RoPE-TOA and the bias inject time into the same product; the bias is quadratic; RoPE-TOA is cheaper.
- `02_answers_rq.tex` — Restate each research question from `\cref{sec:intro:rq}` and answer it directly in 1–3 sentences with a reference to the supporting evidence (table/figure). RQ2: Vanilla joint tax on emitters; that order matches the classical chain; joint RoPE-TOA has no such tax (`tab:exp:attn_em_task`); emitter-only Bias does not match joint Bias. RQ3: both put time into attention; the bias is the quadratic version; RoPE-TOA is cheaper.
