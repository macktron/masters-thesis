...
- `02_answers_rq.tex` — Restate each research question from `\cref{sec:intro:rq}` and answer it directly in 1–3 sentences with a reference to the supporting evidence (table/figure). RQ2: at par when time is in attention; joint RoPE-TOA/Bias match or slightly exceed deint-only on emitters and are slightly higher on modes (`tab:exp:attn_em_task`, `tab:exp:ablation_loss`, `tab:exp:attn_md`); no clear winner vs emitter-only Vanilla; classical mode-on-trains not scored; Vanilla joint still costs emitters. RQ3: both put time into attention; the bias is the quadratic version; RoPE-TOA is cheaper.
