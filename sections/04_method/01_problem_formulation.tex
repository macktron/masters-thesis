...
Emitter identities remain meaningful only within the recording that produced $\mat{X}$.
In particular $\hat{M}$ is not bounded by the catalogue size $M$, so modes absent from training can still form clusters.
In practice $\Enc$ is applied to fixed-length windows of each recording (\cref{sec:method:data:window}), so the estimated symbols are local to a window.
