...
\end{equation}
where the counts $\hat{K}$ and $\hat{M}$ are also estimated from the window.
...
The objective is instead that, within each window, the partitions in \cref{eq:method:estimated_partitions} match the restrictions of the ground-truth partitions in \cref{eq:method:emitter_partition,eq:method:mode_partition}, up to independent permutations of the indices in each coordinate.
Agreement is measured with the permutation-invariant metrics of \cref{sec:method:metrics}.

Both estimates are local to the window, and emitter identities are meaningful only within the recording from which the window was drawn.
