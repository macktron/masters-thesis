...
- `08_baselines.tex` — Matched dual-branch single-task controls (Vanilla with $\Loss_{\mathrm{em}}$ only or $\Loss_{\mathrm{md}}$ only) and optional classical DBSCAN on fixed window features. Inactive branch excluded from the backward pass and not scored. One sentence that emitter-only copies of RoPE-TOA and Bias are scored with the attention variants. Do not describe a flat single-stack encoder unless that run exists.
