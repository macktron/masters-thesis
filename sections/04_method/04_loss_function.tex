...
Emitter identifiers are recording-local in \cref{sec:method:formulation} and are taken to be globally unique within a mini-batch by pairing the recording-local integer with the originating recording identifier, so that $e_{b',n'} = e_{b,n}$ iff the two pulses come from the same physical emitter.
...
Because the loss only consumes pairwise label coincidences, neither the emitter count $K$ nor the catalogue size $M$ appears in \cref{eq:method:supcon}, and modes held out at training time still inherit useful geometry from their neighbours in $\R^{q}$.
