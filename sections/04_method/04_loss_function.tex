...
The training objective is a sum of two \emph{supervised contrastive} \parencite{khosla2020supcon} terms that share their functional form but draw positives from different scopes: within-window for the train-local emitter labels of \cref{sec:method:formulation}, and batch-wide for the global mode labels.
...
The two branches share \cref{eq:method:supcon} and differ only in the index set they feed it.
Let $\vect{z}^{\mathrm{em}}_{b,n}$ and $\vect{z}^{\mathrm{md}}_{b,n}$ denote the projection outputs at position $n \in \{1,\ldots,L\}$ in window $b \in \{1,\ldots,B\}$ of a mini-batch, with labels $e_{b,n}$ and $m_{b,n}$.
The emitter term is computed per window and averaged across the batch with positives restricted to the current window; the mode term pools anchors across the full mini-batch:
\begin{align}
  \Loss_{\mathrm{em}} &\;=\; \frac{1}{B}\sum_{b=1}^{B}
    \Loss_{\mathrm{SC}}\!\bigl(I_b^{\mathrm{em}},\, \{P_{b,n}^{\mathrm{em}}\}\bigr),
    \label{eq:method:loss_emitter}\\
  \Loss_{\mathrm{md}} &\;=\;
    \Loss_{\mathrm{SC}}\!\bigl(I^{\mathrm{md}},\, \{P_{b,n}^{\mathrm{md}}\}\bigr),
    \label{eq:method:loss_mode}
\end{align}
with $I_b^{\mathrm{em}} = \{1,\ldots,L\}$, $P_{b,n}^{\mathrm{em}} = \{\,n' \in I_b^{\mathrm{em}}\setminus\{n\} : e_{b,n'} = e_{b,n}\,\}$ for the emitter aggregation, and $I^{\mathrm{md}} = \{1,\ldots,B\}\!\times\!\{1,\ldots,L\}$, $P_{b,n}^{\mathrm{md}} = \{\,(b',n') \in I^{\mathrm{md}}\setminus\{(b,n)\} : m_{b',n'} = m_{b,n}\,\}$ for the mode aggregation.
Restricting emitter contrasts to a single window enforces the train-local semantics of $e_n$: integer coincidences between emitter labels in different windows are never read by the loss.
Pooling mode anchors over the batch makes the mode term standard SupCon \parencite{khosla2020supcon}; because the loss only consumes pairwise label coincidences, the catalogue size $M$ never appears in \cref{eq:method:supcon}, and modes held out at training time still inherit useful geometry from their neighbours in $\R^{q}$.
