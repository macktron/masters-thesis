...
. Smaller $\tau$ penalize negatives more, while larger $\tau$ might pose some benefits like better generalization by being more forgiving to outliers. However there is no generally established method for selecing the optimal value for the parameter, so it's selected experimantally usually trough a coarse grid search. Some reserach mention that it could be depnedent och batch size \parencite{chen2020simclr}, but likely varies from problem to problem. In our case we have a quite large number of sampler per batc (sequence legnth * batch size) = (2048*8), so heaviliy penalized outliers wiill also bee smoothened as long as they come from a large class. Perhaps a class size weighted temperatrue parameter could be interesting to evaluate.
Both branches use $\tau = 0.15$, the value commonly chosen for normalised contrastive features \parencite{chen2020simclr,khosla2020supcon}.
