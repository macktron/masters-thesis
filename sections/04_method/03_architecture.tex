...
Each branch begins with an affine projection of the same form as \cref{eq:method:input_embed}, mapping $\R^{256} \to \R^{256}$ to decouple the branch representation from the shared trunk output, after which $N_{\mathrm{br}} = 2$ transformer-encoder layers of the same form as the trunk are applied.
The branches retain the trunk width $d_{\mathrm{br}} = 256$; within a branch, \gls{mha} uses $h_{\mathrm{br}} = 4$ heads (per-head dimension $64$) and the \gls{ffn} inner dimension is $d_{\mathrm{ff,br}} = 1024$.
