...
The radius $\varepsilon$ is selected by a grid search on the validation set, separately for each branch, using the clustering metrics of \cref{sec:method:metrics}; the search grid, the selected values, and $\mathrm{minPts}$ are reported in \cref{ch:experiments}.
