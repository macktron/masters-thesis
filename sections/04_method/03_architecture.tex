...
The reduced depth and \gls{ffn} width reflect a deliberate asymmetry: the trunk is intended to carry the heavy lifting of contextual mixing, while the branches only need to specialise the representation to either emitter identity or operating mode. As well the larger head dimension is used to use RoPE with multiple coordinates laters, and to compare this with other archtiecures.
