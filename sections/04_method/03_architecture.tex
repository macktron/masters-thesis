...
Each branch begins with an affine projection of the same form as \cref{eq:method:input_embed}, mapping $\R^{256} \to \R^{256}$ to decouple the branch representation from the shared trunk output, after which $N_{\mathrm{br}} = 2$ transformer-encoder layers of the same form as the trunk are applied. This affince porjection enables different model dimension of the trunk and branchesl
