...
In the original formulation no such factor is needed, because token indices advance in steps of exactly one; our coordinates are normalised physical quantities between zero and one, but whose typical differences differ between the \gls{toa} and the direction components, so we treat $\gamma$ as a hyperparameter and set it separately for each coordinate $f \in \{t,\, u^{x},\, u^{y}\}$: $\gamma_t$ for the \gls{toa} and $\gamma_{u^x}$ and $\gamma_{u^y}$ for the two horizontal direction components. 
