...
A raw-feature \gls{dbscan} control is the Baseline rows of \cref{tab:exp:vanilla}.
...
On windows where frequency and pulse width collide, as in \cref{fig:exp:feat_maxemitters}, a metric on those coordinates alone is not enough; RoPE-TOA closes the remaining emitter tail (\cref{tab:exp:attn_em}).
...
The Vanilla emitter row of \cref{tab:exp:vanilla} is the analogous existence test, on a different corpus and with \gls{dbscan} rather than HDBSCAN.
Gunn et al.\ stop at one partition. The Vanilla mode row of \cref{tab:exp:vanilla} is the addition, and \cref{tab:exp:ablation_loss,fig:exp:kcorr_ablation_em} show that it is not free: joint Vanilla deinterleaves less cleanly than an emitter-only copy of the same encoder.
...
Stacking the two time mechanisms does not close the tail either.
...
Unlike \gls{dec}, DeepCluster, a catalogue softmax, sampled triplets, or a SwAV codebook, the supervised contrastive kernel of \cref{eq:method:supcon} forms all-pairs similarities over the pooled batch; that is a compute cost rather than a clustering-quality claim (\cref{sec:disc:limitations}).
The two branches here are not a hierarchy of one relation. Modes are not subtypes of emitters.
A single embedding cannot keep both similarities, and \cref{tab:exp:ablation_loss} is the empirical trace of that conflict in the shared trunk: joint training costs emitter agreement relative to the emitter-only ceiling.
