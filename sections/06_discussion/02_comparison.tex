...
A raw-feature \gls{dbscan} control is the Feature \gls{dbscan} rows of \cref{tab:exp:vanilla}.
...
Gunn et al.\ stop at one partition. The Vanilla mode row of \cref{tab:exp:vanilla} is the addition, and \cref{tab:exp:ablation_loss,fig:exp:kcorr_ablation_em} show that it is not free: joint Vanilla deinterleaves less cleanly than an emitter-only copy of the same encoder, and a mode-only copy splits emitters.
...
A single embedding cannot keep both similarities, and \cref{tab:exp:ablation_loss} is the empirical trace of that conflict in the shared trunk: the unused head either merges or splits.
