...
- `01_interpretation.tex` — Why does the method work where it does and fail where it does? Tie observed patterns back to the architectural choices in `\cref{ch:method}`. If certain modes are confused, explain why physically. Interpret Mode-only (emitter splits) and RoPE-TOA+Bias (below Vanilla; same overlap and TOA RoPE on trunk as RoPE-TOA).
