...
- `01_interpretation.tex` — Why does the method work where it does and fail where it does? Numbered subsections in RQ order: joint versus single-task first (`sec:disc:joint`), then physical structure in attention (`sec:disc:attention`). Tie observed patterns back to the architectural choices in `\cref{ch:method}`. Interpret Mode-only (emitter splits; `fig:exp:kcorr_ablation_em`, `fig:exp:emb_ablation`) and Emitter-only (mode merges; `fig:exp:kcorr_ablation_md`, `fig:exp:emb_ablation`). Note that RoPE-TOA has no joint emitter tax (cite `sec:exp:attention`; do not add a single-task table). Then RoPE-TOA+Bias (below Vanilla; same overlap and TOA RoPE on trunk as RoPE-TOA).
...
- `03_limitations.tex` — Be honest about (a) dataset limitations (synthetic, narrow, small), (b) methodological assumptions (number of clusters known, deinterleaving solved), (c) compute constraints, (d) generalization to unseen emitters/modes. Vanilla joint tax remains tied to $\lambda_{\mathrm{md}}=1$ on that backbone; do not claim the RoPE-TOA/Bias emitter-only cost is unknown — RoPE-TOA does not show it.
