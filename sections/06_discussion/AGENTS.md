...
- `01_interpretation.tex` — Why does the method work where it does and fail where it does? Tie observed patterns back to the architectural choices in `\cref{ch:method}`. If certain modes are confused, explain why physically. Do not interpret RoPE-TOA+Bias or Mode-only while their table cells are em dashes.
