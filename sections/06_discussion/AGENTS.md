...
- `03_limitations.tex` — Be honest about (a) dataset limitations (synthetic, narrow, small), (b) methodological assumptions (number of clusters known, deinterleaving solved), (c) compute constraints, including $(BL)^{2}$ contrastive compute versus optional materialization, (d) generalization to unseen emitters/modes. Vanilla joint tax remains tied to the unweighted sum $\Loss_{\mathrm{joint}}$ on that backbone. RoPE-TOA does not show that cost; emitter-only Bias does (`tab:app:ari_em`).
