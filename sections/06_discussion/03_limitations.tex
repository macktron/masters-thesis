...
The single-task pipeline of deinterleaving then a classical mode stage on the recovered trains was not scored.
...
RoPE-TOA does not show the same cost: its joint emitter $\ARI$ is at par with an emitter-only copy of that encoder, and slightly higher (\cref{tab:exp:attn_em_task}).
Emitter-only Bias does not match its joint scores on that table.
Each of those comparisons is one training run; a longer schedule or a reweighted joint loss could shrink either gap.
