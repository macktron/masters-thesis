...
- `02_problem_statement.tex` — Precise problem: interleaved PDWs with recording-local emitter labels and global mode labels; learn embeddings clustered per window at inference. Do not claim held-out modulation types unless Chapter 5 evaluates them.
...
- `05_contributions.tex` — Scientific contributions of the work (method, training, evaluation), not an author-credit statement. Bulleted list, each starting with a verb ("We propose ...", "We evaluate ..."). 3–5 bullets. Single-author theses still use this section; "we" is conventional.
...
- Keep claims in the problem statement and objectives consistent with what is actually delivered later. If experiments are incomplete, hedge ("we investigate" rather than "we show").
