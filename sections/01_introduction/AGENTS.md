...
- `02_problem_statement.tex` — One precise paragraph stating the problem: given recordings of PDWs whose pulses carry recording-local emitter labels and global operating-mode labels, learn an embedding from which (i) emitters can be recovered by clustering within each recording at inference time and (ii) the global mode catalogue can be predicted per pulse. Quantify the gap in existing approaches.
