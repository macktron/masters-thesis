...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(8.95, 2.35)}]
    \pgfmathsetmacro{\aoa}{32}            % angle of arrival from boresight
    \pgfmathsetmacro{\arrowL}{0.92}       % length of the incoming-wave ray
    \pgfmathsetmacro{\arcR}{0.50}         % radius of the angle arc
    % subtle dashed frame to mark the diagram as an inset
    \draw[gray!35, dashed, rounded corners=2pt]
      (-0.55, -0.32) rectangle (1.45, 1.24);
    % horizontal receive aperture drawn as a thin filled bar
    \fill[gray!55!black] (-0.40, -0.03) rectangle (0.40, 0.03);
    \node[font=\footnotesize] at (0, -0.22) {Rx};
    % boresight reference perpendicular to the aperture
    \draw[drop] (0, 0.03) -- (0, 1.05);
    ...
    % AOA label placed above the arrow tail so nothing overlaps
    \node[annot] at (0.55, 1.10) {AOA $\theta_n$};
  \end{scope}
