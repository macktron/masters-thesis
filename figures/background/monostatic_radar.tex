...
\begin{tikzpicture}[
  ...
  % --- geometry constants --------------------------------------------
  \def\Rrange{7.2}      % horizontal separation between radar and target
  \def\dishX{0.0}
  \def\targetX{\Rrange}
  ...
  \begin{scope}[shift={(\dishX,0)}]
    ...
    \node[above left=-1pt and -2pt, font=\footnotesize]
      at (-0.05,0.55) {Radar (TX/RX)};
  \end{scope}
  ...
  \begin{scope}[shift={(\targetX,0)}]
    ...
    \node[above, font=\footnotesize] at (0,0.55) {Target};
  \end{scope}
  ...
  % --- range arrow (kept clear of the dish label) --------------------
  \draw[measure] (\dishX + 0.45,-1.55) -- (\targetX - 0.65,-1.55);
  \node[annot, fill=white, inner sep=2pt]
    at ({(\dishX + \targetX)/2}, -1.55) {range $R$};
  ...
  \draw[->] (\txT - 0.8,\tlY) -- (\Rrange + 0.5,\tlY) node[right] {time $t$};
  \draw[txstyle, line width=1pt] (\txT,\tlY) -- (\txT,\tlY + 0.7);
  \node[annot, blue!60!black, above] at (\txT,\tlY + 0.7) {TX pulse};
  \draw[rxstyle, line width=1pt] (\rxT,\tlY) -- (\rxT,\tlY + 0.7);
  \node[annot, orange!70!black, above] at (\rxT,\tlY + 0.7) {RX echo};
  \draw[measure] (\txT,\tlY - 0.55) -- (\rxT,\tlY - 0.55);
  \node[annot, fill=white, inner sep=2pt]
    at ({(\txT + \rxT)/2}, \tlY - 0.55) {$\tau = 2R/c$};
