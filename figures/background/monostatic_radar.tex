...
\begin{tikzpicture}[
  ...
  % --- range arrow ---------------------------------------------------
  \draw[measure] (\dishX + 0.07,-1.85) -- (\targetX,-1.85);
  \node[annot, fill=white, inner sep=2pt]
    at ({(\dishX + \targetX)/2}, -1.85) {range $R$};
  ...
  \draw[->] (\txT - 1.0,\tlY) -- (\Rrange + 0.5,\tlY) node[right] {time $t$};
  \draw[txstyle, line width=1pt] (\txT,\tlY) -- (\txT,\tlY + 0.65);
  \node[annot, blue!60!black, above] at (\txT,\tlY + 0.65) {TX};
  \draw[rxstyle, line width=1pt] (\rxT,\tlY) -- (\rxT,\tlY + 0.65);
  \node[annot, orange!70!black, above] at (\rxT,\tlY + 0.65) {RX (echo)};
  \draw[measure] (\txT,\tlY - 0.5) -- (\rxT,\tlY - 0.5);
  \node[annot, fill=white, inner sep=2pt]
    at ({(\txT + \rxT)/2}, \tlY - 0.5) {$\tau = 2R/c$};
