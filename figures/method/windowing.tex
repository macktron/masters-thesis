...
% s, shown here for the training-time choice s = L/2 (50% overlap). The
% trailing portion of length < L is dropped. Loaded from
...
\begin{tikzpicture}[
  ...
  \def\Sstride{2.4}   % s = L/2 (training stride)
  \def\Ntot{13.4}     % total visible train length
  ...
  % dropped tail overlay (starts at 4s, i.e. just past the last full window)
  \fill[drophatch] (4*\Sstride, -\barH) rectangle (\Ntot, 0);
  \draw[line width=0.5pt, draw=black!65] (4*\Sstride, -\barH) rectangle (\Ntot, 0);
  \node[font=\scriptsize, anchor=north]
       at ({(4*\Sstride+\Ntot)/2}, -\barH-0.02)
       {remainder $<L$ (dropped)};
  ...
  \foreach \xst in {0, \Sstride, 2*\Sstride, 3*\Sstride, 4*\Sstride} {
    \draw[edgeguide] (\xst, 0) -- (\xst, \yzero);
  }
  \draw[edgeguide] (\Lwin, 0) -- (\Lwin, \yzero+\dy);
