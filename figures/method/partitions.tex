% =====================================================================
% Schematic of the two label-induced partitions of a single recording
...
% the emitter partition $\{\mathcal{E}_k\}$ (within-recording) and the mode
...
\begin{tikzpicture}[
  ...
  \def\dx{0.95}
  \def\xlab{-1.4}  % x of the right edge of all left-hand row labels

  % ------------------------------------------------------------------
  % Top row: input recording.
  ...
  % Left-hand label so all three rows share a labelled column.
  \node[rowlabel] (top_lab) at (\xlab, 0)
        {\gls{pdw} sequence};

  % ------------------------------------------------------------------
  % Downward double arrow shared by both partitions.  Reads as
  % "induces the following two partitions".
  % ------------------------------------------------------------------
  \node[font=\Large] (split) at (4*\dx, -1.0) {$\Downarrow$};

  % ------------------------------------------------------------------
  % Emitter partition (within-recording).
  %   E1 (blue):  x1, x3, x7, x8
  %   E2 (red):   x2, x5, x9
  %   E3 (green): x4, x6
  % ------------------------------------------------------------------
  ...
  \node[pulse, em2, modeA] (e2) at (5.0*\dx, \yem) {$\vect{x}_2$};
  \node[pulse, em2, modeB] (e5) at (6.0*\dx, \yem) {$\vect{x}_5$};
  \node[pulse, em2, modeC] (e9) at (7.0*\dx, \yem) {$\vect{x}_9$};

  \node[pulse, em3, modeA] (e4) at (9.0*\dx, \yem) {$\vect{x}_4$};
  \node[pulse, em3, modeC] (e6) at (10.0*\dx, \yem) {$\vect{x}_6$};
  ...
  % Commas between adjacent sets, centred in the inter-group gap.
%  \node[brace] at ($(em1rb.east)!0.5!(em2lb.west)$) {$,$};
%  \node[brace] at ($(em2rb.east)!0.5!(em3lb.west)$) {$,$};

  \node[brace, anchor=east] (em_lb) at ([xshift=-1.5mm]em1lb.west) {$\Bigg\{$};
  \node[brace, anchor=west] (em_rb) at ([xshift=1.5mm]em3rb.east)  {$\Bigg\}$};

  \node[rowlabel] at (\xlab, \yem)
  ...
  \node[pulse, em1, modeB] (m3) at (5.0*\dx, \ymd) {$\vect{x}_3$};
  \node[pulse, em2, modeB] (m5) at (6.0*\dx, \ymd) {$\vect{x}_5$};

  \node[pulse, em3, modeC] (m6) at (8.0*\dx, \ymd) {$\vect{x}_6$};
  \node[pulse, em1, modeC] (m7) at (9.0*\dx, \ymd) {$\vect{x}_7$};
  \node[pulse, em2, modeC] (m9) at (10.0*\dx, \ymd) {$\vect{x}_9$};
