...
% independently with \gls{dbscan} on a per-window basis, yielding the
% discrete partitions of
% \cref{eq:method:estimated_label_pair}.  The
...
% indices, not in the inference pipeline.  The scatter panels show the
% same number of samples (18) but deliberately different partitions
% (3 emitter clusters vs. 2 mode clusters) to emphasize that the two
% clusterings need not coincide.  Loaded from
...
\begin{tikzpicture}[
  ...
  \begin{scope}[xshift=0mm]
    ...
    \node[hdb] (em_hdb) at (0, -45mm)
      {\gls{dbscan} per window};
    \node[outblock] (em_out) at (0, -60mm)
      {Emitter cluster IDs\\$\hat{e}_n \in \{1,\ldots,\hat{K}_{\mathrm{em}}\}$};
  ...
  \begin{scope}[xshift=65mm]
    ...
    % Cluster A (9 samples; the mode partition is coarser than the
    % emitter partition: same number of samples, fewer clusters)
    \draw[contour, color=blue!70!black]
      ($(md_scatter.center)+(-10mm, 3mm)$) ellipse [x radius=7mm, y radius=4mm];
    \foreach \x/\y in {-15/3, -13/5, -11/2, -9/4, -7/3, -12/3.5, -10/1.5, -6/2, -8/5}
      \node[pt, color=blue!70!black] at ($(md_scatter.center)+(\x mm, \y mm)$) {};
    \node[font=\scriptsize, color=blue!70!black]
      at ($(md_scatter.center)+(-10mm, 9mm)$) {$\hat{m}{=}1$};

    % Cluster B (9 samples)
    \draw[contour, color=red!70!black]
      ($(md_scatter.center)+(9mm, -6mm)$) ellipse [x radius=7mm, y radius=4mm];
    \foreach \x/\y in {4/-6, 6/-4, 7/-7, 9/-5, 11/-7, 13/-5, 10/-8, 12/-6.5, 8/-5.5}
      \node[pt, color=red!70!black] at ($(md_scatter.center)+(\x mm, \y mm)$) {};
    \node[font=\scriptsize, color=red!70!black]
      at ($(md_scatter.center)+(9mm, -12mm)$) {$\hat{m}{=}2$};

    \node[hdb] (md_hdb) at (0, -45mm)
      {\gls{dbscan} per window};
    \node[outblock] (md_out) at (0, -60mm)
      {Mode cluster IDs\\$\hat{m}_n \in \{1,\ldots,\hat{M}\}$};
